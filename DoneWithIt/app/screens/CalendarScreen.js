...
function CalendarScreen(navigation) {
    ...
    for (var i = 0; i < global.dayArray.length; i++) {
      if (global.dayArray[i] != null) {
        for (var j = 0; j < global.outfitArray.length; j++) {
          if (
            global.outfitArray[j] != null &&
            global.outfitArray[j].id == global.dayArray[i].outfitId
          ) {
  ...
  function addCalendarDay(o_outfit, o_day) {
    ...
    if (outfit.lastWorn.substring(0, 4) < o_day.year) {
      //just year is larger (more recent)
      ...
      outfit.lastWorn.substring(0, 4) == o_day.year &&
      outfit.lastWorn.substring(5, 7) < o_day.month
    ) {
      //year is equal, just month is larger (more recent)
      outfit.lastWorn = o_day.dateString;
      updateOutfit(
        id,
        global.userEmail,
      ...
      outfit.lastWorn.substring(0, 4) == o_day.year &&
      outfit.lastWorn.substring(5, 7) == o_day.month &&
      outfit.lastWorn.substring(8) < o_day.day
    ) {
      //year and month are same, just day is larger
      outfit.lastWorn = o_day.dateString;
      updateOutfit(
        id,
        global.userEmail,
    ...
    let day = {
      date: o_day,
      outfitId: o_outfit.id,
      notes: "",
    };
    global.dayArray.push(day);
    computeStreak();
  }
  ...
  function removeCalendarDay(o_day, o_outfit) {
    ...
    for (var i = 0; i < global.dayArray.length; i++) {
      if (
        global.dayArray[i] != null &&
        global.dayArray[i].date.dateString === o_day.dateString
      ) {
        deleteDay(global.dayArray[i].id);
        global.dayArray.splice(i, 1);
        ...
        <Portal>
          ...
          >
            ...
            <Button
              ...
              onPress={() => {
                hideModalAll();
                addCalendarDay(c_outfit, currentDay);
              }}
            >
              Confirm
            </Button>
        ...
        <Portal>
          ...
          >
            ...
            <TextInput
              label="Notes"
              value={addNotes}
              onChangeText={(addNotes) => setAddNotes(addNotes)}
            />
            <View style={styles.buttonSpacing}></View>
            <Button
              icon="pencil"
              ...
              onPress={() => {
                setNotesMenu(false);
                setAssigningMenu(true);
                removeCalendarDay(currentDay, c_outfit);
              }}
            >
              Edit
            </Button>
  ...
  return (
    <View>
      <Text style={styles.outfitText}>
        {" "}
        Before using the calendar, be sure to add some outfits to your closet!
      </Text>
    </View>
  );
}
